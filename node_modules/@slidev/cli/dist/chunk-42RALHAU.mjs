...
async function load2(filepath, themeMeta, content) {
  ...
  for (let iSlide = 0; iSlide < data.slides.length; ) {
    ...
    const path8 = resolve(dir, srcExpression);
    const raw = await fs.readFile(path8, "utf-8");
...
import fs3, { promises as fsPromises } from "fs";
...
var defaultRcPath = path4.join(home || "~/", ".nirc");
...
function npmRunPath(options = {}) {
  ...
  let cwdPath = path5.resolve(cwdString);
  const result = [];
  ...
  result.push(path5.resolve(cwdString, execPath, ".."));
  return [...result, path_].join(path5.delimiter);
}
function npmRunPathEnv(_a = {}) {
  var _b = _a, { env = process3.env } = _b, options = __objRest(_b, ["env"]);
...
var getEnv = ({ env: envOption, extendEnv, preferLocal, localDir, execPath }) => {
  const env = extendEnv ? __spreadValues2(__spreadValues2({}, process4.env), envOption) : envOption;
  if (preferLocal) {
...
var handleArguments = (file, args, options = {}) => {
  ...
  if (process4.platform === "win32" && path6.basename(file, ".exe") === "cmd") {
    args.unshift("/q");
  }
  return { file, args, options, parsed };
};
var handleOutput = (options, value, error) => {
  if (typeof value !== "string" && !Buffer2.isBuffer(value)) {
...
async function detect({ autoInstall, cwd }) {
  ...
  let packageJsonPath;
  if (lockPath)
    packageJsonPath = path7.resolve(lockPath, "../package.json");
  else
    packageJsonPath = await findUp("package.json", { cwd });
  if (packageJsonPath && fs32.existsSync(packageJsonPath)) {
    try {
      const pkg = JSON.parse(fs32.readFileSync(packageJsonPath, "utf8"));
  ...
  if (!agent && lockPath)
    agent = LOCKS[path7.basename(lockPath)];
...
async function getThemeMeta(name, path8) {
  if (!fs4.existsSync(path8))
    return {};
  if (path8) {
    const { slidev = {}, engines = {} } = await fs4.readJSON(path8);
